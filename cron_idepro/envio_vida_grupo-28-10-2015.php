...
FROM s_de_em_cabecera t0 inner join  s_de_em_detalle t1 on t0.id_emision = t1.id_emision inner join s_cliente t2 on t1.id_cliente = t2.id_cliente inner join s_usuario t3 on t0.id_usuario = t3.id_usuario left outer join s_departamento t4 on t4.id_depto = t3.id_depto left outer join s_agencia t5 on t5.id_agencia = t3.id_agencia inner join s_de_cot_cabecera t6 on t0.id_cotizacion = t6.id_cotizacion

WHERE t0.emitir =1 and t0.anulado = 0 and t6.vg = 1 and t1.titular = 'DD'
						ORDER BY
							t0.no_emision ASC"; 				
//echo $selectdes;
...
$sfile=$scarpeta."/Llamadas_".$date1.".xls"; //ruta del archivo a generar
$fp=fopen($sfile,"w");
