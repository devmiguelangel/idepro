...
class CertificateQuery extends CertificateHtml {
	...
	private function set_query_de_sc_mo () {
		...
		from
		    ...
		    s_compania as sc on (sc.id_compania=sefc.id_compania)
			    inner join
			s_sgc_home as sh ON (sh.id_ef = ef.id_ef and sh.producto='".$this->product."')
			    inner join
	...
	//QUERYS SLIP PRODUCTO EXTRA VIDA GRUPO
	...
	private function set_query_de_em_mo () {
		$this->sqlPo = "select 
			...
			(case sde.moneda
		        when 'BS' then (18000*stc.valor_boliviano)
		        when 'USD' then 18000
	...
	//QUERYS CERTIFICADOS PRODUCTO EXTRA EMISION VIDA GRUPO
	private function set_query_de_em_pec () {	//DESGRAVAMEN
		$this->sqlPo = "select 
		    ...
		    sde.emitir,
		    sde.fecha_emision,
			date_format(sde.fecha_emision, '%d') as dia,
            date_format(sde.fecha_emision, '%m') as mes,
            date_format(sde.fecha_emision, '%Y') as anio,
		    sde.id_compania,
		    sde.facultativo,
		    sde.motivo_facultativo,
		    sde.prima_total,
		...
		if (($this->rsPo = $this->cx->query($this->sqlPo, MYSQLI_STORE_RESULT))) {
			if ($this->rsPo->num_rows === 1) {
				...
				$this->sqlDt = "select 
				    ...
				    date_format(scl.fecha_nacimiento, '%d-%m-%Y') as fecha_nacimiento,
				    scl.lugar_nacimiento,
				    concat(scl.ci, ' ', scl.complemento, ' ', sdep.codigo) as ci_document,
				    (case sdep.codigo
				        when 'LP' then 'La Paz'
				        when 'CB' then 'Cochabamba'
				        when 'OR' then 'Oruro'
			...
			if($this->rsPo->num_rows === 1){
				...
				$this->sqlDt="select
							...
							from
							  ...
							  inner join s_au_tipo_vehiculo as autv on (autv.id_tipo_vh=auc.id_tipo_vh)
							  inner join s_au_marca as aumr on (aumr.id_marca=auc.id_marca)
							  inner join s_au_modelo as aumod on (aumod.id_modelo=auc.id_modelo)
							where
	...
	private function set_query_au_em () {		// Automotores
		$this->sqlPo = "select
						  ...
						  sclie.paterno as cl_paterno,
						  sclie.materno as cl_materno,
						  sclie.nombre as cl_nombre,
						  sclie.ap_casada as cl_ap_casada,
						  (case sclie.avenida
						     when 'AV' then 'Avenida'
							 when 'CA' then 'Calle'
						   end) as cl_avc,
						  sclie.direccion as cl_direccion,
						  sclie.no_domicilio as cl_no_domicilio,
						  sclie.localidad as cl_localidad,
						  sclie.telefono_domicilio as cl_tel_domicilio,
						  sclie.telefono_oficina as cl_tel_oficina,
						  sclie.telefono_celular as cl_tel_celular,
						  sclie.ci as cl_ci,
						  sclie.complemento as cl_complemento,
						  case sclie.genero
						     when 'M' then 'Masculino'
							 when 'F' then 'Femenino'
						  end as genero,
						...
						from
						  ...
						  left join s_au_facultativo as saf ON (saf.id_emision = auec.id_emision)
						  inner join s_forma_pago as sfp ON (sfp.id_forma_pago = auec.id_forma_pago)
						  inner join s_poliza as sp ON (sp.id_poliza = auec.id_poliza and sp.producto = '".$this->product."')
						  inner join s_departamento as sdep ON (sdep.id_depto = sclie.extension)
						  inner join s_departamento as sdeu ON (sdeu.id_depto = su.id_depto)
						where
		...
		$this->sqlPo="select 
					...
					from
						...
						s_ocupacion as socu ON (socu.id_ocupacion = trdclt.id_ocupacion
						    ...
						    inner join
						s_sgc_home as sh on (sh.id_ef=trdc.id_ef and sh.producto='".$this->product."')	
					where
	...
	private function set_query_trd_em () {		// Todo Riesgo Domiciliario
		$this->sqlPo = 'select 
			...
			(case scl.tipo
				when 0 then concat(scl.ci, scl.complemento)
				when 1 then scl.ci
			end) as cl_ci,
			sdep.departamento as expedido,
			(case scl.avenida
		...
		from
			...
			s_ocupacion as ocup ON (ocup.id_ocupacion = scl.id_ocupacion and ocup.producto="'.$this->product.'")
			    inner join
			s_poliza as sp ON (sp.id_poliza = stre.id_poliza and sp.producto = "'.$this->product.'")	 
		where
