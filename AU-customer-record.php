...
$arrAU = array(0 => 0, 1 => 'R', 2 => 'Error: No se pudo registrar el Arrendatario');
			...
			if($flag === TRUE){
				...
				if($vc[0] === TRUE){
					...
					$sql = 'UPDATE s_au_cot_cliente 
						SET `razon_social` = "'.$dc_company_name.'", `paterno` = "'.$dc_lnpatern.'", 
							...
							`complemento` = "'.$dc_comp.'", `id_ocupacion` = "'.$dc_occupation.'", `desc_ocupacion` = "'.$dc_desc_occ.'", `genero` = "'.$dc_gender.'", `telefono_domicilio` = "'.$dc_phone_1.'", 
					...
					$sql = 'INSERT INTO s_au_cot_cliente (`id_cliente`, `id_ef`, `tipo`, `razon_social`, `paterno`, `materno`, `nombre`, `ap_casada`, `fecha_nacimiento`, `ci`, `extension`, `complemento`, `id_ocupacion`, `desc_ocupacion`, `genero`, `telefono_domicilio`, `telefono_oficina`, `telefono_celular`, `email`) 
						VALUES ("'.$idClient.'", "'.$idef.'", '.(int)$dc_type_client.', "'.$dc_company_name.'", 
							"'.$dc_lnpatern.'", "'.$dc_lnmatern.'", "'.$dc_name.'", "'.$dc_lnmarried.'", "'.$dc_birth.'", 
							"'.$dni.'", '.$ext.', "'.$dc_comp.'", "'.$dc_occupation.'", "'.$dc_desc_occ.'", "'.$dc_gender.'", "'.$dc_phone_1.'", "'.$dc_phone_office.'", "'.$dc_phone_2.'", "'.$email.'") ;';
				}
				...
				if($link->query($sql) === TRUE){
					...
					if($link->query($sqlIn) === TRUE){
						$arrAU[0] = 1;
						$arrAU[1] = 'au-quote.php?ms='.$ms.'&page='.$page.'&pr='.$pr.'&idc='.base64_encode($idc);
						$arrAU[2] = 'Titular registrado con Exito';
					}else {
						$arrAU[2] = 'No se pudo registrar los datos del Seguro Solicitado';
					}
				}else {
					$arrAU[2] = 'No se pudo registrar el Arrendatario';
				}
			}
		}else{
