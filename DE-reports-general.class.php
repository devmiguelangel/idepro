...
class ReportsGeneralDE{
	...
	private function set_variable($data){
		...
		$this->data['vida-grupo'] = $this->cx->real_escape_string(trim($data['r-vg']));
	...
	private function set_query_de_report(){
		...
		$this->sql = "select 
			...
			@fum:=(if(sde.emitir = true, 
				datediff(sde.fecha_emision, sdp.fecha_creacion),
				datediff(curdate(), sdp.fecha_creacion))) as fum,
		...
		where
			sef.id_ef = '".$this->data['idef']."'
				...
				and sdc.vg like '%".$this->data['vida-grupo']."%' ";
		if($this->token === 'RG'){
...
<?php
		...
		while($this->row = $this->rs->fetch_array(MYSQLI_ASSOC)){
			...
			$tipo_cambio = $this->cx->get_rate_exchange(true);
...
<?php
						if ($this->token === 'RG') {
							echo '<td ' . $rowSpan . '>' . $this->row['dias_proceso'] . '</td>
								<td ' . $rowSpan . '>' . $this->row['dias_ultima_modificacion'] . '</td>
								';
						}
?>
            <td <?=$rowSpan;?>><?=htmlentities($this->row['duracion_caso'].' días', ENT_QUOTES, 'UTF-8');?></td>
<?php
						if ($this->token === 'RG') {
							$solicitud = 'N/A';
							$bg_sol = '';
