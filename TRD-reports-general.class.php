...
class ReportsGeneralTRD{
	...
	private function set_query_trd_report(){
		...
		$this->sql = "select 
		    ...
		    sfp.forma_pago as r_forma_pago, stre.no_operacion, 
		    (case scl.tipo
		        when 0 then 'NATURAL'
		...
		while($this->row = $this->rs->fetch_array(MYSQLI_ASSOC)){
			...
			from
    			...
    			s_departamento as sdep ON (sdep.id_depto = strd.departamento)
				    inner join
				s_poliza as sp ON (sp.id_poliza = stre.id_poliza)	
			where
...
?>
		<tr style=" <?=$bg;?> " class="row-au" rel="0" data-nc="<?=base64_encode($this->row['ide']);?>" data-token="<?=$this->dataToken;?>" data-pr="<?=base64_encode($this->rowpr['idPr']);?>" data-issue="<?=base64_encode(0);?>">
            ...
            <td>
			  <?php
                 if($this->rowpr['moneda']==='Bs'){
				    echo number_format($this->rowpr['pr_valor_asegurado']*$tc,2,'.',',').' Bs';
				 }elseif($this->rowpr['moneda']==='Usd'){
					echo number_format($this->rowpr['pr_valor_asegurado'],2,'.',',').' USD'; 
				 }
			  ?> 
            </td>
