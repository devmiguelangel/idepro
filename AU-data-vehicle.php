	...
	$("#dv-value-insured").keyup(function(e){
		...
		if(isNaN(amount) === true) {
			...
			if(amount > max_amount) {
				$("#mess-amount").fadeIn();
				$("#dv-vehicle").attr('disabled', true);
			} else {
				$("#mess-amount").fadeOut();
				$("#dv-vehicle").removeAttr('disabled');
			}
		}
	});
	
	$('#dv-type-vehicle').change(function(){
	   var data = $('#dv-type-vehicle option:selected').attr('data');
	   //alert(data);
	   if((data==1)||(data==2)||(data==3)||(data==5)||(data==8)||(data==12)){
		   $("#dv-category option[data='1']").attr('disabled',false);
		   $("#dv-category option[data='1']").prop('selected',true);
		   $('#dv-category').trigger('change');
		   $("#dv-category option[data='2']").attr('disabled',true);
	   }else if((data==4)||(data==6)||(data==7)||(data==9)||(data==10)||(data==11)||(data==13)||(data==14)||(data==15)){
		   $("#dv-category option[data='2']").attr('disabled',false);
		   $("#dv-category option[data='2']").prop('selected',true);
		   $('#dv-category').trigger('change');
		   $("#dv-category option[data='1']").attr('disabled',true);
	   }
	   //var vehicle = $('#dv-type-vehicle option:selected').text();
	});

<?php
if(isset($_GET['idVh'])){ 
?>	
	$('#dv-category').change(function(){
	   	$('#dv-type-vehicle option[value=""]').prop('selected',true);
	});
<?php
}
?>	
...
if(($rsTv = $link->get_type_vehicle($_SESSION['idEF'])) !== FALSE){
	...
	while($rowTv = $rsTv->fetch_array(MYSQLI_ASSOC)){
		...
		if($rowTv['id_vh'] === $dv_type_vehicle) {
			echo '<option data="'.$i.'" value="'.base64_encode($rowTv['id_vh']).'" selected>'.$rowTv['vehiculo'].'</option>';
		} else {
			echo '<option data="'.$i.'" value="'.base64_encode($rowTv['id_vh']).'">'.$rowTv['vehiculo'].'</option>';
		}
	}
}
?>
...
for($i = 0; $i < count($arr_category); $i++){
	...
	if($category[0] === $dv_category) {
		echo '<option data="'.($i+1).'" value="'.base64_encode($category[0]).'" selected>'.$category[1].'</option>';
	} else {
		echo '<option data="'.($i+1).'" value="'.base64_encode($category[0]).'">'.$category[1].'</option>';
	}
}
?>
			</select>
...
if(($rowYear = $link->get_year_cot($_SESSION['idEF'])) !== FALSE){
	...
	$year_max = (int)$rowYear['anio_max']+1;
...
?>
		<div class="content-input">
			<input type="text" id="dv-value-insured" name="dv-value-insured" autocomplete="off" value="<?=$dv_value_insured;?>" class="required number fbin">
		</div><br>
		<!--
        <div id="mess-amount" class="au-mess" style=" <?=$display_value;?> ">Vehículos cuyo valor excedan los <?=$max_amount;?> USD requieren aprobación de la Compañia de Seguros</div>
        -->
        <div id="mess-amount" class="au-mess" style=" <?=$display_value;?> ">El valor asegurado no debe ser mayor a <?=$max_amount;?> USD.</div>
