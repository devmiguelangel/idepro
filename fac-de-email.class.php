	...
	public function send_mail_fac($ide, $emailCC = ''){
		...
		from
			...
			s_estado as sds ON (sds.id_estado = sdp.id_estado)
				left join
			s_cm_respuesta as scr ON (scr.id_emision = sde.id_emision and scr.id_cliente = sc.id_cliente)
				left join
			...
			if($this->rs->num_rows > 0){
				...
				if($this->rs->data_seek(0) === TRUE){
					...
					$this->sqlc = 'SELECT sc.correo, sc.nombre 
						...
						WHERE (sc.producto = "DE" or sc.producto = "FDE") 
								AND sef.id_ef = "' . $this->row['idef'] . '" 
...
?>
<div style="width:600px; border:1px solid #CCCCCC; color:#000000; font-weight:bold; font-size:12px; text-align:left;">
	<div style="padding:5px 10px; background:#009646; color:#FFFFFF;">N&Uacute;MERO DE EMISI&Oacute;N</div>
    <div style="padding:5px 10px;"><?=$this->row['prefijo'] . '-' . $this->row['no_emision'];?></div>

	<div style="padding:5px 10px; background:#009646; color:#FFFFFF;">ESTADO DE LA SOLICITUD</div>
    <div style="padding:5px 10px;"><?=$state[1];?></div>
    <div style="padding:5px 10px; background:#009646; color:#FFFFFF;">MOTIVO PENDIENTE</div>
    <div style="padding:5px 10px;"><?=$this->row['estado_pendiente'];?></div>
    <div style="padding:5px 10px; background:#009646; color:#FFFFFF;">DETALLES: </div>
    <div style="padding:5px 10px;"><?=$this->row['p_observacion'];?></div>
    <div style="padding:5px 10px; background:#009646; color:#FFFFFF;">RESPUESTA DEL OFICIAL DE CR&Eacute;DITO: </div>
    <div style="padding:5px 10px;"><?=$this->row['p_obs_respuesta'];?></div>

    <div style="padding:5px 10px; background:#009646; color:#FFFFFF;">DEUDOR: </div>
    <div style="padding:5px 10px;"><?=$arrCl[1];?></div>
<?php
		if(count($arrCl) === 2){
?>
	<div style="padding:5px 10px; background:#009646; color:#FFFFFF;">CODEUDOR: </div>
    <div style="padding:5px 10px;"><?=$arrCl[2];?></div>
<?php
		}
?>
