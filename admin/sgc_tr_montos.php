...
<script type="text/javascript">
  ...
  $(function(){
    ...
    <?php if($msg!=''){ ?>
		 ...
		 setTimeout( "$(location).attr('href', 'index.php?l=tr_montos&var=<?php echo $var;?>');",5000 );
	<?php }?>
...
if(isset($_SESSION['usuario_sesion']) && isset($_SESSION['tipo_sesion'])) {
	...
	if(isset($_POST['username'])) {
		...
		if(validar_login($conexion)) {
			...
			header('Location: index.php?l=tr_montos&var=trd');
			exit;
		} else {
					...
					<div class="da-panel-header" style="text-align:right; padding-top:5px; padding-bottom:5px;">
						<ul class="action_user">
							<li style="margin-right:6px;">
							   <a href="?l=tr_montos&var='.$_GET['var'].'&crear=v" class="da-tooltip-s various" title="Añadir nuevo registro">
				...
				while($regief = $resef->fetch_array(MYSQLI_ASSOC)){		
					$selectFor="select
								...
								from
								  s_sgc_home
								where
								  producto='TRD' and id_ef='".$regief['id_ef']."';";
					...
					<div class="da-panel collapsible">
						<div class="da-panel-header">
							<span class="da-panel-title" style="font-size:11.5px">
								...
								<b>'.$regief['nombre'].'</b> - Administrar parametros del Producto Todo Riesgo Domiciliario
							</span>
						</div>
							...
							<table class="da-table">
								<thead>
									<tr style="font-size:11.5px;">
										...
										<th style="text-align:center;"><b>N&deg; Registros</b></th>
								  ...
								  if($num>0){
										while($regi = $res->fetch_array(MYSQLI_ASSOC)){
											echo'<tr style="font-size:11.5px;">
											   ...
											   echo'<td>
													   <ul class="action_user">
														  <li style="margin-right:5px;"><a href="?l=tr_montos&idhome='.base64_encode($regi['id_home']).'&id_ef='.base64_encode($regief['id_ef']).'&editar=v&var='.$_GET['var'].'" class="edit da-tooltip-s" title="Editar"></a></li>';
...
function agregar_nuevos_montos($id_usuario_sesion, $tipo_sesion, $usuario_sesion, $id_ef_sesion, $conexion){
	...
	if(isset($_POST['accionGuardar'])) {
			...
			//$monto_facu = mysql_real_escape_string($_POST['txtMontFacu']);
			
			$id_new_home = generar_id_codificado('@S#1$2013');					
			//METEMOS LOS DATOS A LA BASE DE DATOS
			$insert ="INSERT INTO s_sgc_home(id_home, id_ef, producto, limite_cotizacion, max_cotizacion_usd,  max_emision_usd, edad_max, edad_min, max_detalle, facturacion, implante, id_usuario) "
				    ."VALUES('".$id_new_home."', '".$_POST['idefin']."', 'TRD', ".$caducidadcotiz.", ".$montocotizusd.", ".$montoemiusd.", ".$edadmax.", ".$edadmin.", ".$numregi.", ".$facturacion.", ".$implante.", '".$id_usuario_sesion."')";
			//echo $insert;
			...
			if($conexion->query($insert)===TRUE){
			    ...
			    header('Location: index.php?l=tr_montos&var='.$_GET['var'].'&op=1&msg='.base64_encode($mensaje));
			    exit;
			} else {
				$mensaje="Hubo un error al ingresar los datos, consulte con su administrador ".$conexion->errno.": ".$conexion->error;
			    header('Location: index.php?l=tr_montos&var='.$_GET['var'].'&op=2&msg='.base64_encode($mensaje));
				exit;
			}
...
<script type="text/javascript">
  $(function(){
	  ...
	  $('#btnCancelar').click(function(e){
		  ...
		  $(location).attr('href', 'index.php?l=tr_montos&var='+variable); 
	  });  
  });
</script>
<?php    
		  ...
		  echo'<div class="da-panel" style="width:600px;">
				<div class="da-panel-header">
					<span class="da-panel-title">
						...
						Crear Montos Todo Riesgo Domiciliario
					</span>
				</div>
				<div class="da-panel-content">
...
<?php    
	...
	$select = "select 
				...
				where
					sh.producto = 'TRD' and sh.id_home = '".$idhome."'
	...
	if($rs = $conexion->query($select, MYSQLI_STORE_RESULT)){
				...
				if($num>0) {
					...
					//if(isset($_POST['txtMontFacu'])) $txtMontFacu = $_POST['txtMontFacu']; else $txtMontFacu = $fila['monto_facultativo'];
					  ...
					  echo'<div class="da-panel" style="width:600px;">
							<div class="da-panel-header">
								<span class="da-panel-title">
									<img src="images/icons/black/16/pencil.png" alt="" />
									Editar Datos Todo Riesgo Domiciliario
								</span>
							</div>
							<div class="da-panel-content">
								<form class="da-form" name="frmDatosAdmin" action="" method="post" id="frmDatosAdmin">
									...
									<div class="da-form-row">
										<label style="width:190px; text-align:right; margin-right:10px;"><b>Facturación</b></label>
										<div class="da-form-item">
											<ul class="da-form-list inline">';
											   if($factura==1){	
					...
					header('Location: index.php?l=tr_montos&var='.$_GET['var']);
				}
	}else{
		echo"<div style='font-size:8pt; text-align:center; margin-top:20px; margin-bottom:15px; border:1px solid #C68A8A; background:#FFEBEA; padding:8px; width:600px;'>Error en la consulta: "."\n ".$conexion->errno . ": " .$conexion->error."</div>";
	}
