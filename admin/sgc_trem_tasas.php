	...
	if(isset($_POST['accionGuardar'])) {
		...
		if($errFlag) {
			...
			while($j<=$_POST['cant_tasas']){
				...
				$update = "UPDATE s_tasa_trm SET tasa=".$tasa." WHERE id_tasa=".$id_tasa." and id_ef_cia='".$id_ef_cia."';";
   ...
   $(function(){
	   $("a[href].eliminar").click(function(e){
		   ...
		   jConfirm("¿Esta seguro de eliminar la tasa?", "Eliminar registro", function(r) {
				//alert(r);
				if(r) {
						var dataString ='id_tasa='+id_tasa+'&id_ef_cia='+id_ef_cia+'&opcion=elimina_tasa_trem';
...
<script type="text/javascript">
  ...
  $(function(){
    ...
    <?php if($msg!=''){ ?>
		 ...
		 setTimeout( "$(location).attr('href', 'index.php?l=trem_tasas&listartasas=v&id_ef_cia=<?php echo $id_ef_cia;?>&entidad=<?php echo $entidad;?>&compania=<?php echo $compania;?>&var=<?php echo $var;?>');",5000 );
	<?php }?>
	...
	if(isset($_POST['accionGuardar'])) {
		...
		if($errFlag) {
			...
			mostrar_crear_tasas($id_usuario_sesion, $tipo_sesion, $usuario_sesion, $conexion, $errArr);
			  ...
			  $insert = "INSERT INTO s_tasa_trm(id_tasa, id_ef_cia, tasa, anio) "
