 ...
 <?php 
  ...
  $(function(){
    ...
    <?php if($msg!=''){ ?>
		 ...
		 setTimeout( "$(location).attr('href', 'index.php?l=estados&var=<?php echo $var;?>');",5000 );
	<?php }?>
...
echo'<div class="da-panel collapsible">
		<div class="da-panel-header" style="text-align:right; padding-top:5px; padding-bottom:5px;">
			<ul class="action_user">
				<li style="margin-right:6px;">
				   <a href="?l=estados&var='.$_GET['var'].'&crear=v" class="da-tooltip-s various fancybox.ajax" title="Añadir Estado">
	...
	while($regief = $resef->fetch_array(MYSQLI_ASSOC)){	
			$selectFor="select
						...
						where
						  se.id_ef='".$regief['id_ef']."' and se.producto=sh.producto;";
								...
								while($regi = $res->fetch_array(MYSQLI_ASSOC)){
									echo'<tr id="del-'.$regi['id_estado'].'">
											...
											<td class="da-icon-column">
											   <ul class="action_user">
												  <li style="margin-right:5px;"><a href="?l=estados&idestado='.base64_encode($regi['id_estado']).'&id_ef='.base64_encode($regief['id_ef']).'&editar=v&var='.$_GET['var'].'" class="edit da-tooltip-s" title="Editar"></a></li>
												  <li><a href="#" class="eliminar da-tooltip-s" title="Eliminar" id="'.$regi['id_estado'].'|'.$regief['id_ef'].'"></a></li>';
										  echo'</ul>	
											</td>
										</tr>';
								}
...
function agregar_nuevo_estado($id_usuario_sesion, $tipo_sesion, $usuario_sesion, $id_ef_sesion, $conexion){
	...
	if(isset($_POST['btnGuardar'])) {
			...
			if($conexion->query($insert)===TRUE){
			    ...
			    header('Location: index.php?l=estados&var='.$_GET['var'].'&op=1&msg='.base64_encode($mensaje));
			    exit;
			} else {
				$mensaje="Hubo un error al ingresar los datos, consulte con su administrador "."\n ".$conexion->errno.": " . $conexion->error;
			    header('Location: index.php?l=estados&var='.$_GET['var'].'&op=2&msg='.base64_encode($mensaje));
				exit;
			}
...
function mostrar_crear_estado($id_usuario_sesion, $tipo_sesion, $usuario_sesion, $id_ef_sesion, $conexion){
	...
	<script type="text/javascript">
       $(document).ready(function() {
		   ...
		   $('#btnCancelar').click(function(){   
			   ...
			   $(location).prop('href','index.php?l=estados&var='+variable);
		   });   
	   });
    </script>
 ...
 echo'<div class="da-panel collapsible">
		<div class="da-panel-header" style="text-align:right; padding-top:5px; padding-bottom:5px;">
			<ul class="action_user">
				<li style="margin-right:6px;">
				   <a href="?l=estados&var='.$_GET['var'].'" class="da-tooltip-s" title="Volver">
