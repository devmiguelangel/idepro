										  ...
										  if($num>0){
													   ...
													   echo'</td>
															<td class="da-icon-column">
															   <ul class="action_user">';
																   ...
																   echo'<li style="margin-right:5px;"><a href="?l=tr_tasas&id_ef_cia='.base64_encode($regi['id_ef_cia']).'&entidad='.base64_encode($regief['nombre']).'&compania='.base64_encode($regi['compania']).'&listartasas=v&var='.$_GET['var'].'" class="add_mod da-tooltip-s" title="Editar Tasas"></a></li>';
											 ...
											 echo'<tr><td colspan="7">
													  <div class="da-message warning">
														 No existe registros, probablemente se debe a:
														 <ul>
															<li>La Compañia de Seguros no esta activada</li>
															<li>La Compañia asignada a la Entidad Financiera no esta activada</li>
															<li>El producto no existe en la Compañia asignada a la Entidad Financiera</li>
														  </ul>
													  </div>
												  </td></tr>';
										  }
	...
	if($resu = $conexion->query($selectTs,MYSQLI_STORE_RESULT)){
		...
		echo'<div class="da-panel collapsible">
				<div class="da-panel-header" style="text-align:right; padding-top:5px; padding-bottom:5px;">
					<ul class="action_user">
						<li style="margin-right:6px;">
							 <a href="?l=tr_tasas&var='.$_GET['var'].'&list_compania=v" class="da-tooltip-s" title="Volver">
						  ...
						  <div class="da-form-item large" style="margin:0px;">
							<table class="da-table">
								<thead>
									<tr>
									  <th><b>Tasa Año</b></th>
									  <th><b>Tasa Restante</b></th>
									  <th><b>Tasa Estandar</b></th>
									  <th>&nbsp;</th>
									</tr>
								</thead>
								<tbody>';
	...
	$('#frmTasas').submit(function(e){
		...
		$(this).find('.required').each(function() {
			if(tasa_anio!=''){
			   ...
			   sum++;
			   $('#errortasa').show('slow');
			   $('#errortasa').html('ingrese tasa año');  
			}
			if(tasarestan!=''){
				if(tasarestan.match(/^[0-9\.]+$/)){
				...
				sum++;
				$('#errortasarestan').show('slow');
				$('#errortasarestan').html('ingrese tasa restante');
		    }
			if(tasaestand!=''){
				if(tasaestand.match(/^[0-9\.]+$/)){
				...
				sum++;
				$('#errortasaestand').show('slow');
				$('#errortasaestand').html('ingrese tasa estandar');
		    }	
		});
		if(sum==0){
			/*if(num_regi<3){
					...
					<table class="da-table">
						...
						echo'<tr>
								...
								<td>
								 ...
								 <span class="errorMessage" id="errortasarestan"></span>
								 
								</td>
								<td>
								 <input type="text" name="txtTasaEstand" id="txtTasaEstand" value="" class="required" style="width:125px;"/>
								 <span class="errorMessage" id="errortasaestand"></span>
								</td>
							 </tr>';
