...
<script type="text/javascript">
  ...
  $(function(){
    ...
    <?php if($msg!=''){ ?>
		 ...
		 setTimeout( "$(location).attr('href', 'index.php?l=th_montos&var=<?php echo $var;?>');",5000 );
	<?php }?>
...
if(isset($_SESSION['usuario_sesion']) && isset($_SESSION['tipo_sesion'])) {
	...
	if(isset($_POST['username'])) {
		...
		if(validar_login($conexion)) {
			...
			header('Location: index.php?l=th_montos&var=th');
			exit;
		} else {
...
<?php
	...
	if($resef = $conexion->query($selectEf,MYSQLI_STORE_RESULT)){
					...
					<div class="da-panel-header" style="text-align:right; padding-top:5px; padding-bottom:5px;">
						<ul class="action_user">
							<li style="margin-right:6px;">
							   <a href="?l=au_montos&var='.$_GET['var'].'&crear=v" class="da-tooltip-s various" title="Añadir nuevo registro">
			...
			if($num_reg>0){	 
				while($regief = $resef->fetch_array(MYSQL_ASSOC)){		
					$selectFor="select
								   id_home,
								   producto,
								   limite_cotizacion,
								...
								from
								  s_sgc_home
								where
								  producto='TH' and id_ef='".$regief['id_ef']."';";
					...
					<div class="da-panel collapsible">
						<div class="da-panel-header">
							<span class="da-panel-title" style="font-size:11.5px;">
								...
								<b>'.$regief['nombre'].'</b> - Administrar parametros del Producto Tarjetahabiente
							</span>
						</div>
							...
							<table class="da-table">
								<thead>
									<tr style="font-size:11.5px;">
										<th style="text-align:center;"><b>Caducidad Cotizacion (dias)</b></th>
										<th style="text-align:center;"><b>Edad Minima</b></th>
										<th style="text-align:center;"><b>Edad Maxima</b></th>
										<th style="text-align:center;"><b>Facturación</b></th>
								  ...
								  if($num>0){
										while($regi = $res->fetch_array(MYSQLI_ASSOC)){
											echo'<tr style="font-size:11.5px;">
													...
													<td>
													   <ul class="action_user">
														  <li style="margin-right:5px;"><a href="?l=th_montos&idhome='.base64_encode($regi['id_home']).'&id_ef='.base64_encode($regief['id_ef']).'&editar=v&var='.$_GET['var'].'" class="edit da-tooltip-s" title="Editar"></a></li>';
			  ...
			  echo'<div class="da-message info">
					 No existe ningun registro, probablemente se debe a:
					 <ul>
						<li>La Entidad Financiera no tiene asignado el producto Automotores</li>
						<li>La Entidad Financiera no esta activado</li>
						<li>La Entidad Financiera no esta creada</li>
					  </ul>
				 </div>';
			}
	}else{
...
function agregar_nuevos_montos($id_usuario_sesion, $tipo_sesion, $usuario_sesion, $id_ef_sesion, $conexion){
	...
	if(isset($_POST['accionGuardar'])) {
			...
			$numautos = $conexion->real_escape_string($_POST['txtNumAutos']);
			...
			$insert ="INSERT INTO s_sgc_home(id_home, id_ef, producto, limite_cotizacion, max_cotizacion_usd,  max_emision_usd, edad_max, edad_min, max_detalle, monto_facultativo, anio, facturacion, implante, id_usuario) "
				    ."VALUES('".$id_new_home."', '".$_POST['idefin']."', 'AU', ".$caducidadcotiz.", ".$montocotizusd.", ".$montoemiusd.", ".$edadmax.", ".$edadmin.", ".$numautos.", ".$monto_facu.", ".$antiguedad.", ".$facturacion.", ".$implante.", '".$id_usuario_sesion."')";
			...
			if($conexion->query($insert)===TRUE){
			    ...
			    header('Location: index.php?l=au_montos&var='.$_GET['var'].'&op=1&msg='.base64_encode($mensaje));
			    exit;
			} else {
			    $mensaje="Hubo un error al ingresar los datos, consulte con su administrador ".$conexion->errno.": ".$conexion->error;
			    header('Location: index.php?l=au_montos&var='.$_GET['var'].'&op=2&msg='.base64_encode($mensaje));
				exit;
			}
...
<script type="text/javascript">
  $(function(){
	  ...
	  $('#btnCancelar').click(function(e){
		  ...
		  $(location).attr('href', 'index.php?l=au_montos&var='+variable); 
	  });  
  });
</script>
<?php    
		  ...
		  echo'<div class="da-panel" style="width:600px;">
				...
				<div class="da-panel-content">
					<form class="da-form" name="frmDatosCrea" action="" method="post" id="frmDatosCrea">
						...
						<div class="da-form-row">
							<label style="width:190px; text-align:right; margin-right:10px;"><b>Numero de Autos</b></label>
							<div class="da-form-item large">
								<input class="textbox required" type="text" name="txtNumAutos" id="txtNumAutos" style="width: 200px;" value="" autocomplete="off"/>
								<span class="errorMessage" id="errornumautos"></span>
							</div>
						</div>
						<div class="da-form-row">
							<label style="width:190px; text-align:right; margin-right:10px;"><b>Antig&uuml;edad del vehiculo</b></label>
							<div class="da-form-item large">
								<input class="textbox required" type="text" name="antiguedad" id="antiguedad" style="width: 200px;" value="" autocomplete="off"/>
...
function editar_datos_admin($id_usuario_sesion, $tipo_sesion, $usuario_sesion, $id_ef_sesion, $conexion) {
	...
	if(isset($_POST['btnUsuario'])) {
            ...
            $update = "UPDATE s_sgc_home SET limite_cotizacion=".$caducidadcotiz.", edad_max=".$edadmax.", edad_min=".$edadmin.", facturacion=".$factura.", implante=".$implante.", certificado_provisional=".$cert_prov.", modalidad=".$modalidad." WHERE id_home='".$idhome."' and id_ef='".$id_ef."' LIMIT 1;";
            //echo $update;
            

            if($conexion->query($update)===TRUE){
                $mensaje="Se actualizo correctamente los datos del formulario";
			    header('Location: index.php?l=th_montos&var='.$_GET['var'].'&op=1&msg='.base64_encode($mensaje));
			    exit;
            } else{
                $mensaje="Hubo un error al ingresar los datos, consulte con su administrador ".$conexion->errno.": ".$conexion->error;
			    header('Location: index.php?l=th_montos&var='.$_GET['var'].'&op=2&msg='.base64_encode($mensaje));
				exit;
            }
...
<?php    
	...
	$select = "select 
				...
				where
					sh.producto = 'TH' and sh.id_home = '".$idhome."'
	...
	if($rs = $conexion->query($select, MYSQLI_STORE_RESULT)){
			...
			if($num>0) {
				...
				header('Location: index.php?l=th_montos&var='.$_GET['var']);
			}
	}else{
		echo"<div style='font-size:8pt; text-align:center; margin-top:20px; margin-bottom:15px; border:1px solid #C68A8A; background:#FFEBEA; padding:8px; width:600px;'>Error en la consulta: "."\n ".$conexion->errno . ": " .$conexion->error."</div>";
	}
