<?php
  ...
  if($_POST['opcion']=='certmedico'){
	 ...
	 if($num_regi==0){
	  ...
	  $myarray = array(1 => $_POST['txtRgMinBs'], 2 => $_POST['txtRgMaxBs'], 3 => $_POST['txtRgMinUsd'], 4 => $_POST['txtRgMaxUsd']);
	  $myJson = $conexion->real_escape_string(json_encode($myarray));
	  $update = "update s_de_producto_extra set rango='".$myJson."', pr_hospitalario=".$_POST['txtHospitalario'].", pr_vida=".$_POST['txtVida'].", pr_cesante=".$_POST['txtCesantia'].", pr_prima=".$_POST['txtPrima']." where id_pr_extra='".$_POST['id_pr_extra']."' and id_ef_cia='".$_POST['id_ef_cia']."';";
	   if($conexion->query($update)===TRUE){
		  echo 1; 
	   }else{
		  echo 2; 
