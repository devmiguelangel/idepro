						...
						<div class="da-panel collapsible" style="width:900px;">
							<div class="da-panel-header">
								<span class="da-panel-title">
									...
									<b>'.$regief['nombre'].'</b> - Editar Polizas</b>
								</span>
							</div>
   ...
   if($res = $conexion->query($selectPol,MYSQLI_STORE_RESULT)){		  
				...
				<div class="da-panel-header" style="text-align:right; padding-top:5px; padding-bottom:5px;">
					<ul class="action_user">
						<li style="margin-right:6px;">
						   <a href="?l=des_poliza&var='.$_GET['var'].'&list_producto=v" class="da-tooltip-s" title="Volver">
						   <img src="images/retornar.png" width="32" height="32"></a>
						</li>
						<li style="margin-right:6px;">
						   <a href="?l=des_poliza&id_ef_cia='.$_GET['id_ef_cia'].'&id_ef='.$_GET['id_ef'].'&entidad='.$_GET['entidad'].'&compania='.$_GET['compania'].'&producto_nombre='.$_GET['producto_nombre'].'&producto_code='.$_GET['producto_code'].'&crear=v&var='.$_GET['var'].'" class="da-tooltip-s various" title="Añadir nueva poliza">
		...
		<div class="da-panel collapsible">
			<div class="da-panel-header">
				<span class="da-panel-title">
					<img src="images/icons/black/16/list.png" alt="" />
					<b>'.$entidad.' - Listado de Polizas</b>
				</span>
			</div>
			<div class="da-panel-content">
				<table class="da-table">
					<thead>
						<tr>
						  <th style="text-align:center;"><b>No Póliza</b></th>
						  <th style="text-align:center;"><b>Moneda</b></th>
						  <th style="text-align:center;"><b>Fecha Inicial</b></th>
						  <th style="text-align:center;"><b>Fecha Final</b></th>
						  <th style="text-align:center;"><b>Producto</b></th>
						  <th></th>
						</tr>
					</thead>
...
function agregar_nueva_poliza($id_usuario_sesion, $tipo_sesion, $usuario_sesion, $id_ef_sesion, $conexion){
	...
	if(isset($_POST['btnGuardar'])) {
			...
			$insert ="INSERT INTO s_poliza(id_poliza, no_poliza, moneda, fecha_ini, fecha_fin, producto, id_ef_cia) "
				    ."VALUES('".$id_new_poliza."', '".$num_poliza."', '".$moneda."', '".$fecha_ini."', '".$fecha_fin."', '".$producto."', '".$id_ef_cia."')";
...
<script type="text/javascript">
  $(function(){
	 ...
	 $('#frmDatosPoliza').submit(function(e){
		  ...
		  var moneda = $('#moneda option:selected').prop('value');
		  ...
		  $(this).find('.required').each(function(){
			   ...
			   if(moneda!=''){
				   $('#errormoneda').fadeOut('slow');
			   }else{
				   sum++; 
				   $('#errormoneda').fadeIn('slow');
				   $('#errormoneda').html('seleccione moneda');  
			   }
			   if(fecha_ini!=''){
...
<?php  
  ...
  echo'<div class="da-panel collapsible">
		<div class="da-panel-header" style="text-align:right; padding-top:5px; padding-bottom:5px;">
			<ul class="action_user">
				<li style="margin-right:6px;">
				   <a href="?l=des_poliza&var='.$_GET['var'].'&listarpolizas=v&id_ef_cia='.$_GET['id_ef_cia'].'&entidad='.$_GET['entidad'].'&compania='.$_GET['compania'].'&id_ef='.$_GET['id_ef'].'&producto_nombre='.$_GET['producto_nombre'].'&producto_code='.$_GET['producto_code'].'" class="da-tooltip-s" title="Volver">
				   <img src="images/retornar.png" width="32" height="32"></a>
				</li>
			</ul>
		</div>
  ...
  echo'<div class="da-panel" style="width:600px;">
			<div class="da-panel-header">
				<span class="da-panel-title">
					...
					<div style="margin-left:20px;">Agregar Nueva Poliza</div>
				</span>
			</div>
			<div class="da-panel-content">
				<form class="da-form" name="frmDatosPoliza" action="" method="post" id="frmDatosPoliza">
					...
					<div class="da-form-row">
						<label style="text-align:right;"><b>No Poliza</b></label>
					...
					<div class="da-form-row">
						<label style="text-align:right;"><b>Moneda</b></label>
						<div class="da-form-item small">
							<select id="moneda" name="moneda" class="requerid">
							   <option value="">seleccione</option>
							   <option value="Bs">Boliviano</option>
							   <option value="Usd">Dolar</option>
							</select>
							<span class="errorMessage" id="errormoneda"></span>
						</div>
					</div>
					<div class="da-form-row">
						<label style="text-align:right;"><b>Fecha Inicial</b></label>
						<div class="da-form-item large">
					...
					<div class="da-form-row">
						<label style="text-align:right;"><b>Fecha Final</b></label>
						<div class="da-form-item large">
					...
					<div class="da-button-row">
					   <input type="submit" value="Guardar" class="da-button green" name="btnGuardar" id="btnGuardar"/><input type="hidden" id="var" value="'.$_GET['var'].'"/>
...
function editar_datos_poliza($id_usuario_sesion, $tipo_sesion, $usuario_sesion, $id_ef_sesion, $conexion) {
	...
	if(isset($_POST['btnGuardar'])) {
            ...
            $update = "UPDATE s_poliza SET no_poliza='".$num_poliza."', fecha_ini='".$fecha_ini."', fecha_fin='".$fecha_fin."', moneda='".$moneda."' WHERE id_poliza='".$idpoliza."' and id_ef_cia='".$id_ef_cia."';";
            //echo $update;
	...
	if($rs = $conexion->query($select, MYSQLI_STORE_RESULT)){
			...
			if($num>0) {
				...
				<div class="da-panel-header" style="text-align:right; padding-top:5px; padding-bottom:5px;">
					<ul class="action_user">
						<li style="margin-right:6px;">
						   <a href="?l=des_poliza&var='.$_GET['var'].'&listarpolizas=v&id_ef_cia='.$_GET['id_ef_cia'].'&entidad='.$_GET['entidad'].'&compania='.$_GET['compania'].'&id_ef='.base64_encode($fila['id_ef']).'&producto_nombre='.$_GET['producto_nombre'].'&producto_code='.$_GET['producto_code'].'" class="da-tooltip-s" title="Volver">
						   <img src="images/retornar.png" width="32" height="32"></a>
						</li>
					</ul>
				</div>
				...
				if(isset($_POST['txtProducto'])) $txtProducto = $_POST['txtProducto']; else $txtProducto = $fila['producto'];
				  ...
				  echo'<div class="da-panel" style="width:600px;">
							<div class="da-panel-header">
								<span class="da-panel-title">
									<img src="images/icons/black/16/pencil.png" alt="" />
									<b>'.$entidad.' - '.$compania.'</b> - Editar Poliza
								</span>
							</div>
							<div class="da-panel-content">
								<form class="da-form" name="frmDatosPoliza" action="" method="post" id="frmDatosPoliza">
