...
class SibasDB extends MySQLi
{
	...
	public
		...
		$typeProperty = array(
					...
					2 => 'BLDN|Edificio',
					3 => 'COND|Condominio',
					4 => 'LOCL|Local/Oficina Comercial',
					5 => 'STOC|Almacen industrial',
					6 => 'PLIN|Planta industrial'),
		$useProperty = array(
					'INT' => 'Inmueble Industrial',
					'OTH' => 'OTRO'
					),
		$stateProperty = array(
					0 => 'FINS|Terminado',
    ...
    public function checkPolicyFac ($data = array(), $client = array())
    {
        if (count($data) === 6) {
            $this->sql = 'select
            ...
            where sdec.emitir = false
                  ...
                  and (if(sdf.aprobado is null,
                       ...
                       if(sdf.aprobado = "SI" and sdec.emitir = false,
                          true,
                          false))) = true
                  and sdec.id_ef = "' . base64_decode($data['id_ef']) . '"
                  and sdec.prefijo = "' . $data['prefix'] . '"
                  and sdec.monto_solicitado = ' . $data['amount'] . '
                  and sdec.moneda = "' . $data['currency'] . '"
                  and sdec.plazo = ' . $data['term'] . '
                  and sdec.tipo_plazo = "' . $data['type_term'] . '"
            ;';

            if (($this->rs = $this->query($this->sql, MYSQLI_STORE_RESULT)) !== false) {
                if ($this->rs->num_rows === count($client)) {
                    $flag = true;
                    $k = 0;
	...
	{
		$this->sql = 'select 
			sp.id_poliza, sp.no_poliza, sp.moneda
		from
			s_poliza sp
				inner join
	...
	{
		$arrTypeIssue = array (
			0 => array ('min' => 0, 		'max' => 18000),
			1 => array ('min' => 18001, 	'max' => 50000),
	...
	public function get_max_amount_optional($idef, $product = 'AU')
	{
		$this->sql = 'select 
				...
				sh.anio as max_anio,
				sh.max_emision_usd as max_value
			from
				s_sgc_home as sh
					inner join
				s_entidad_financiera as sef ON (sef.id_ef = sh.id_ef)
		...
		$this->sql = 'select 
			...
			where
				sac.id_cotizacion = "'.$idc.'"
					...
					when "AN" then if('.$year.' < 5, '.$year.', 5)
					when "PC" then if('.$year.' < 5, '.$year.', 5)
					when "CO" then if('.$year.' < 5, '.$year.', 5)
				end)
	...
	public function get_prima_au($plazo,$tipo_plazo,$valor_asegurado,$tasaf,$year){
		$tm = 0;
		$tf = 0;
		$prima = 0;
		if($year>5) $year = 5;
